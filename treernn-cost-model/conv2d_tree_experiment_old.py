...
class Tree(object):
    ...
    # def __init__(self, depth, attributes):
    #     self.parent = None
    #     self.depth = depth
...
def conv2d_no_batching(N, H, W, CO, CI, KH, KW, stride, padding):
    ...
    # print(s[conv])
    # print(s[conv].op)
    # op_json = tvm.save_json(s[conv].op)
    # print(op_json)

    # print(s[conv].op.axis)
    # ir = tvm.lower(s, simple_mode=True)
    ...
    # stmt = ana_lower(s, args, simple_mode=True)
    # print(stmt)

    # You could encode these in C++.
    # Perform softmax in C++, too.
    ...
    root.print()
        ...
        #     trees[depth] = tree
        #     continue
        # elif depth - 1 in trees:
        #     last_tree.children.append(tree)
        #     trees[depth - 1].children.append(tree)
    ...
    # features = get_itervar_feature(s, args, take_log=True)
    # features = get_buffer_curve_sample_flatten(s, args)
    # print(features)
    # features = get_itervar_feature(s, args)
    # print(features)

    # print(get_itervar_feature(s, args))
    # features = get_itervar_feature_flatten(s, args)
    # print(len(features))

    # exit(1)
    # ir = tvm.lower(s, [raw_data, kernel, conv], simple_mode=True)
    # print(ir)
...
def main(feature_type):
    ...
    logging.getLogger('autotvm').addHandler(logging.FileHandler(log_file))
    ...
    dispatch_context = autotvm.apply_history_best(config_file)
    best_config = dispatch_context.query(task.target, task.workload)
    print("\nBest config:")
    print(best_config)

    # apply history best from config file
    with autotvm.apply_history_best(config_file):
