...
# from tvm.autotvm.tuner import XGBTuner, GATuner, RandomTuner, GridSearchTuner, TreeLSTMTuner
...
def tune_tasks(tasks,
    ...
    for i, tsk in enumerate(reversed(tasks)):
        ...
        # create tuner
        if tuner == 'xgb-itervar':
            tuner_obj = XGBTuner(tsk, feature_type='itervar')
        elif tuner == 'xgb-curve':
            tuner_obj = XGBTuner(tsk, feature_type='curve')
        elif tuner == 'xgb-knob':
            tuner_obj = XGBTuner(tsk, feature_type='knob')
        elif tuner == 'ga':
...
# def tune_and_evaluate(tuning_opt, network, target):
def tune_and_evaluate(tuning_option, network, target, dtype, log_file):
    ...
    #     total_length += len(task.config_space)
    # print('Total config space:', total_length)

    # NOTE: No tuning, just testing.
    # run tuning tasks
    # print("Tuning...")
...
def main(feature_type):
    ...
    log_file = '{}-{}.log'.format(network, feature_type)
    dtype = 'float32'

    tuning_option = {
        'log_filename': log_file,

        'tuner': 'xgb-{}'.format(feature_type),
        'n_trial': 1000,
        'early_stopping': 600,
    ...
    # Tune and evaluate.
    tune_and_evaluate(tuning_option, network, target, dtype, log_file)
