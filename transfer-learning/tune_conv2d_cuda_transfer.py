...
def main_transfer(feature_type, source, target, history_file, n_trial=1000):
    logging.getLogger('autotvm').setLevel(logging.DEBUG)
    logging.getLogger('autotvm').addHandler(logging.StreamHandler(sys.stdout))

    for i in range(5):
        print('MAIN ITERATION {}.'.format(i))
        main_body(feature_type, source, target, history_file, n_trial)
        # Test with n_trial=20 to verify things work.
        # main_body(feature_type, source, target, history_file, n_trial=20)

def main_body(feature_type, source, target, history_file, n_trial=1000):
    assert source in conv_configs, "Source '{}' is not a convolution config.".format(source)
    assert target in conv_configs, "Target '{}' is not a convolution config.".format(target)
    assert os.path.exists(history_file), "History file '{}' does not exist.".format(history_file)
...
if __name__ == "__main__":
    ...
    main_transfer(feature_type, source, target, history_file)
